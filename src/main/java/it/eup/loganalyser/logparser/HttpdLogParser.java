...
public class HttpdLogParser {
  ...
  public HttpdLogParser(String logFormat) {
    ...
    registerParser(TokenBuilder.newStringBuilder("%{User-Agent}i").withConverter(new StringConverter(1024))
        .withValueConsumer((x, y) -> x.setUserAgent(y)));
    registerParser(
        TokenBuilder.newStringBuilder("%{Referer}i").withConverter(new StringConverter(2048)).withValueConsumer((x, y) -> x.setReferer(y)));
    registerParser(
        TokenBuilder.newStringBuilder("%{Host}i").withConverter(new StringConverter(1024)).withValueConsumer((x, y) -> x.setHostname(y)));
    registerParser(
        TokenBuilder.newStringBuilder("%v").withConverter(new StringConverter(1024)).withValueConsumer((x, y) -> x.setHostname(y)));
